# Certifications – Dhanshri Patel

This section showcases my professional certifications and continuous learning journey.

## Designations
- **CSM – Certified Scrum Master**
- **FLMI Level 1 – Fellow LOMA Management Institute**
- **ALMI – Associate LOMA Management Institute**
- **AWS Foundational Cloud Practitioner**
- **SafePOPM – Product Owner/Product Manager (In Progress)**
- **Gen AI – Product Owner**
- IBM Product Owner

## External Certificates
- LOMA 281 – Meeting Customer Needs with Insurance and Annuities
- LOMA 291 – Improving the Bottom Line: Insurance Company Operations
- LOMA 302 – The Policy Lifecycle: Insurance Administration
- LOMA 308 – The Business of Insurance: Applying Financial Concepts
- LOMA 321 – Insurance Marketing: Connecting with Customers

## Internal Certificates
- Introduction to Scrum Master Training – LearnQuest (Coursera)
- Introduction to Cloud Computing – IBM (Coursera)

---

This folder highlights my commitment to professional growth, blending QA expertise with cloud, Agile, and AI knowledge.
